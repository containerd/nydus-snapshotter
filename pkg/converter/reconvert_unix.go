...
// DefaultIndexConvertFuncOld wraps containerd's converter to handle Nydus blob reconversion.
func DefaultIndexConvertFunc(layerConvertFunc converter.ConvertFunc, docker2oci bool, platformMC platforms.MatchComparer) converter.ConvertFunc {
	...
	logrus.Debugf("Using DefaultIndexConvertFuncOld in reconvert_unix.go")

	fn := converter.IndexConvertFuncWithHook(layerConvertFunc, docker2oci, platformMC, hooks)

	return func(ctx context.Context, cs content.Store, desc ocispec.Descriptor) (*ocispec.Descriptor, error) {
		logrus.Debugf("DefaultIndexConvertFuncOld called for desc mediaType=%s digest=%s", desc.MediaType, desc.Digest.String())
