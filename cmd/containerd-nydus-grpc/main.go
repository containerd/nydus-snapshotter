...
func main() {
	...
	app := &cli.App{
		...
		Action: func(c *cli.Context) error {
			...
			if flags.Args.SanpshotterConfig != "" {
				if err := config.LoadShotterConfigFile(flags.Args.SanpshotterConfig, &snapshotterCfg); err != nil {
					return errors.Wrapf(err, "failed to load snapshotter config file")
				}
				if err := config.SetStartupParameter(&snapshotterCfg.StartupParameterCfg, &cfg); err != nil {
