...
// Config corresponds to oerdctl.toml .
// See docs/config.md .
type Config struct {
...
func initRootCmdFlags(rootCmd *cobra.Command, tomlPath string) (*pflag.FlagSet, error) {
	...
	if r, err := os.Open(tomlPath); err == nil {
		...
		if err := dec.Decode(cfg); err != nil {
			return nil, fmt.Errorf("failed to load oerdctl config (not daemon config) from %q (Hint: don't mix up daemon's `config.toml` with `oerdctl.toml`): %w", tomlPath, err)
		}
	...
	// -a is aliases (conflicts with oerdctl images -a)
	...
	// -n is aliases (conflicts with oerdctl logs -n)
	...
	rootCmd.PersistentFlags().String("data-root", cfg.DataRoot, "Root directory of persistent oerdctl state (managed by nerdctl, not by containerd)")
...
func newApp() (*cobra.Command, error) {
	...
	if v, ok := os.LookupEnv("OERDCTL_TOML"); ok {
		tomlPath = v
	}

	short := "oerdctl is a command line to record accessed files during container lifecycle based on nerdctl"
	long := fmt.Sprintf(`%s

Config file (same with $NERDCTL_TOML): %s
`, short, tomlPath)
...
// unknownSubcommandAction is needed to let `oerdctl system non-existent-command` fail
