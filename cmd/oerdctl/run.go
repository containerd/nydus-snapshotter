...
func newRunCommand() *cobra.Command {
	...
	case "windows":
		longHelp += "\n"
		longHelp += "WARNING: `oerdctl run` is experimental on Windows and currently broken (https://github.com/containerd/nerdctl/issues/28)"
	case "freebsd":
		longHelp += "\n"
		longHelp += "WARNING: `oerdctl run` is experimental on FreeBSD and currently requires `--net=none` (https://github.com/containerd/nerdctl/blob/main/docs/freebsd.md)"
	}
...
func setCreateFlags(cmd *cobra.Command) {
	...
	// entrypoint StringArray is an internal implementation to support `oerdctl compose` entrypoint yaml filed with multiple strings
	...
	// #fanotify flags
	cmd.Flags().Int64("wait-time", 0, "The time to record accessed files.")
	cmd.Flags().String("wait-line", "", "Record accessed files until get the line from stdout.")
	cmd.Flags().Bool("wait-signal", false, "Record accessed files until get the signal.")
	cmd.Flags().String("output", "accessed_files.txt", "The path to store accessed files.")
	// #endfanotify
}
...
func runAction(cmd *cobra.Command, args []string) error {
	...
	container, gc, fanotifiyCtx, err := createContainer(ctx, cmd, client, args, platform, flagI, flagT, flagD)
	if err != nil {
		if gc != nil {
			defer gc()
		}
	...
	var waiterCtx fanotify.WaiterContext
	var task containerd.Task
	if fanotifiyCtx.WaitSignal || fanotifiyCtx.WaitLine != "" || fanotifiyCtx.WaitTime > 0 {
		if err = fanotifiyCtx.PrepareWaiter(ctx, &waiterCtx); err != nil {
			return err
		}

		if task, err = container.NewTask(ctx, *waiterCtx.IoCreator); err != nil {
			return err
		}
	} else {
	...
	if waiterCtx.ReadCloser != nil {
		waiterCtx.ReadCloser.RegisterCloser(func() { // Ensure to close IO when stdin get EOF
	...
	if flagT {
		if err := tasks.HandleConsoleResize(ctx, task, *waiterCtx.Console); err != nil {
	...
	if err := fanotifiyCtx.StartWaiter(ctx, container, task, &waiterCtx, flagD, rm); err != nil {
		return err
	}

	if waiterCtx.ExitCode != 0 {
		return ExitCodeError{
			exitCode: int(waiterCtx.ExitCode),
		}
	}
...
func createContainer(ctx context.Context, cmd *cobra.Command, client *containerd.Client, args []string, platform string, flagI, flagT, flagD bool) (containerd.Container, func(), *fanotify.Context, error) {
	...
	hookOpt := withNerdctlOCIHook(cmd)
	...
	fanotifyOpts, fanotifiyCtx, err := fanotify.GenerateFanotifyOpts(ctx, cmd, flagI, flagT)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	if !imageless {
		...
		for ind, env := range ensured.ImageConfig.Env {
			if strings.HasPrefix(env, "PATH=") {
				break
			} else if ind == len(ensured.ImageConfig.Env)-1 {
				opts = append(opts, oci.WithDefaultPathEnv)
			}
		}
	} else {
...
func withNerdctlOCIHook(cmd *cobra.Command) oci.SpecOpts {
