...
type snapshotter struct {
	...
	syncRemove           bool
	cleanupOnClose       bool
	// containerdClient is used to query containerd for snapshot metadata
	// when recovering from desynchronization scenarios
...
func NewSnapshotter(ctx context.Context, cfg *config.SnapshotterConfig) (snapshots.Snapshotter, error) {
	...
	// Initialize containerd client for lazy parent recovery.
	// This allows the snapshotter to query containerd for snapshot
	// metadata when recovering from desynchronization scenarios where
	// local BoltDB is missing entries.
	var containerdClient *client.Client
	containerdAddr := cfg.ContainerdAddress
	if containerdAddr == "" {
		containerdAddr = defaultContainerdAddress
	}
	containerdClient, err = client.New(containerdAddr)
	if err != nil {
		log.L.Errorf("Failed to connect to containerd at %s for lazy parent recovery: %v", containerdAddr, err)
		return nil, err
	}
	log.L.Infof("Connected to containerd at %s for lazy parent recovery", containerdAddr)
